 ...
 *
 */
function rushmark_shared_counts_header( $output, $location ) {
	if( 'after_content' === $location )
		$output = '<h3>Share this Article</h3>' . $output;
	return $output;
}
add_filter( 'shared_counts_display', 'rushmark_shared_counts_header', 10, 2 );

/**
 * Simple email button
 * Does not require loading JS
 */
function rushmark_shared_counts_email_link( $link, $id ) {
	if( 'email' !== $link['type'] )
		return $link;
...
add_filter( 'shared_counts_link', 'rushmark_shared_counts_email_link', 10, 2 );

/**
 * Shared Counts Services
 *
 */
function rushmark_shared_counts_services( $services, $location ) {
	if( 'after_content' !== $location )
		return $services;
...
add_filter( 'shared_counts_display_services', 'rushmark_shared_counts_services', 10, 2 );

/**
 * Shared Counts Locations
 *
 */
function rushmark_shared_counts_locations( $locations ) {
	$locations['before']['hook'] = 'rushmark_entry_header_share';
	$locations['after']['hook'] = 'rushmark_entry_footer_share';
	$locations['after']['style'] = 'button';
	return $locations;
}
add_filter( 'shared_counts_theme_locations', 'rushmark_shared_counts_locations' );
 ...
 *
 */
function rushmark_production_url( $url = false ) {
 ...
 * @return array
 */
function rushmark_production_url_share_count_api( $params ) {
	$params['url'] = rushmark_production_url( $params['url'] );
	return $params;
}
add_filter( 'shared_counts_api_params', 'rushmark_production_url_share_count_api' );
 ...
 * @return array
 */
function rushmark_production_url_share_count_link( $link ) {
	$exclude = array( 'print', 'email' );
	if( ! in_array( $link['type'], $exclude ) )
		$link['link'] = rushmark_production_url( $link['link'] );
	return $link;
}
add_filter( 'shared_counts_link', 'rushmark_production_url_share_count_link' );
