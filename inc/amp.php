 ...
 */
function rushmark_amp_class( $default, $active, $state ) {
	$output = '';
	if( rushmark_is_amp() ) {
		$output .= sprintf(
			' [class]="%s"',
 ...
 */
function rushmark_amp_toggle( $state = '', $disable = array() ) {
	if( ! rushmark_is_amp() )
		return;
 ...
 */
function rushmark_amp_nav_dropdown( $theme_location = false, $depth = 0 ) {
	...
	return rushmark_amp_toggle( $key ) . rushmark_amp_class( 'submenu-expand', 'expanded', $key );
}
