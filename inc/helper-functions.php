...
global $wp_embed;
add_filter( 'rushmark_the_content', array( $wp_embed, 'run_shortcode' ), 8 );
add_filter( 'rushmark_the_content', array( $wp_embed, 'autoembed'     ), 8 );
add_filter( 'rushmark_the_content', 'wptexturize'        );
add_filter( 'rushmark_the_content', 'convert_chars'      );
add_filter( 'rushmark_the_content', 'wpautop'            );
add_filter( 'rushmark_the_content', 'shortcode_unautop'  );
add_filter( 'rushmark_the_content', 'do_shortcode'       );
 ...
 */
function rushmark_first_term( $args = [] ) {
 ...
 */
function rushmark_class( $base_classes, $optional_class, $conditional ) {
 ...
 */
function rushmark_bg_image_style( $image_id = false, $image_size = 'full' ) {
	if( !empty( $image_id ) )
 ...
 *
 */
function rushmark_icon( $atts = array() ) {
 ...
 *
 */
function rushmark_has_action( $hook ) {
	ob_start();
	do_action( $hook );
	$output = ob_get_clean();
	return !empty( $output );
