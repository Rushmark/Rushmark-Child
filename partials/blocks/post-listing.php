...
$loop = new WP_Query( rushmark_post_listing_args( $settings ) );
if( ! $loop->have_posts() )
	return;
...
echo '<section class="' . join( ' ', $classes ) . '"' . $anchor . '>';
	...
	while( $loop->have_posts() ): $loop->the_post();
		$partial = rushmark_post_listing_partial( $settings['layout'], $loop->current_post );
		get_template_part( 'partials/archive', $partial );
	endwhile;
	wp_reset_postdata();
	rushmark_post_listing_footer( $block );
echo '</section>';
